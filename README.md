...
- **Docker Desktop** (chạy Postgres 16) · **uv** (`brew install uv` hoặc `pip install uv`, để tạo venv Python 3.12) · **Apify token**.

## Chạy nhanh (Quickstart — macOS)
...
# 2) Database (mở Docker Desktop trước: `open -a Docker`, chờ daemon sẵn sàng)
...
# 5) Xem lại các run
python main.py status                    # thống kê run trong terminal
```
...
| Lệnh                                                          | Chức năng                                          |
| -------------------------------------------------------------- | ---------------------------------------------------- |
| `python main.py migrate`                                     | Tạo / nâng cấp schema DB                          |
| `python main.py crawl --config config.json`                  | Chạy crawl (config 1 object hoặc mảng nhiều bộ) |
| `python main.py crawl --config config.json --resume <RUN_ID>` | Tiếp tục run bị dừng                             |
| `python main.py status [--run <RUN_ID>]`                     | Xem trạng thái run                                 |
